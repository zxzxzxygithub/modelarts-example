...
def check_input_shape(inputs, signature):
    ...
    assert len(inputs) == len(
        signature['inputs']), 'Input number mismatches with ' \
    ...
    for input, sig_input in zip(inputs, signature['inputs']):
        ...
        assert len(input.shape) == \
               len(sig_input[
                       'data_shape']), 'Shape dimension of input %s mismatches with ' \
                                       'signature. %d expected but got %d.' \
                                       % (sig_input['data_name'],
...
class DLSMXNetBaseService(MXNetBaseService):
    ...
    def __init__(self, model_name, model_dir, manifest, gpu=None):
        ...
        data_names = []
        data_shapes = []
        self._signature['inputs'] = [{'data_name': 'images', 'data_shape':
            self._signature['inputs'][0]['data_shape']}]
    ...
    def _preprocess(self, data):
        ...
        for idx, img in enumerate(data):
            ...
            img_arr = mx.image.color_normalize(img_arr, mean=rgb_mean,
                                               std=rgb_std)
    ...
    def _postprocess(self, data):
        ...
        top_prob = map(lambda x: int(x.asscalar()), sorted_prob[0:top_k])
        return {"predicted_label": class_names[top_prob[0]],
                "scores": [[class_names[i], float(data[0, i].asscalar())] for i
                           in top_prob]}
